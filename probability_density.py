        ...
        while(True):                                            # loop 1
            ...
            for ship in range(len(remaining_ships)):
                ...
                while(True):                                    # loop 2
                    ...
                    orientation = random.randint(0,2)           # horizontal = 0, vertical = 1
                    if(not generate_and_check_positions(board, x, y, orientation, ship)):
...
# False return means the starting coordinates are not valid
def generate_and_check_positions(board, x, y, orientation, ship):
    ...
    if (orientation == 0):                          # change y
        for i in range(remaining_ships[ship]):
            if(not present_in_predicted([x,y+pivot+dir*i])):
                return False
            predicted_ships_locations[ship].append([x,y+pivot+dir*i])
            if(board[x][y+pivot+dir*(i+1)]=='0'):
                if(dir == -1):
        ...
        for i in range(remaining_ships[ship]):
            if(not present_in_predicted([x+pivot+dir*i,y])):
                return False
            predicted_ships_locations[ship].append([x+pivot+dir*i,y])
            if(board[x+pivot+dir*(i+1)][y]=='0'):
                if(dir == -1):
...
def check_overhang_or_miss_on_position(board,x,y, ship, orientation):
...
def play(board):
    ...
    while True:
        ...
        remaining_ships = []
        sunk_ships = utils.get_sunk_ships(board)
        for i in range(1,6):
            if not i in sunk_ships:
                remaining_ships.append(utils.ships[i][3])
